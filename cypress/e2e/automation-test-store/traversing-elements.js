...
describe("Traversing DOM elements in Cypress", () => {
  ...
  it("nextUntil() to get all of the next sibling DOM elements within elements until another element", () => {
    cy.get("#coffee").contains("Coffee").nextUntil("#milk");
  });
  ...
  it("prev() to get the previous sibling DOM element within elements", () => {
    //cy.get("#sugar").contains("Sugar").prev().should("contain", "espresso"); [Wrong statement]
    cy.get("#sugar").contains("Sugar").prev().contains("Espresso");
  });

  it("prevAll() to get all previous sibling DOM elements within elements", () => {
    cy.get(".sales").contains("Sales").prevAll().should("have.length"), "2";
  });

  it("prevUntil() to get all previous sibling DOM elements within elements until other element", () => {
    cy.get("#veggie").prevUntil("#fruits").should("have.length", 5);
    // cy.get("#veggie").prevUntil("#cherries");
  });
